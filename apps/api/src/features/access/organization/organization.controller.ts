...
export class OrganizationController {
  create = asyncHandler(async (req: Request, res: Response) => {
    ...
    console.log(
      `[OrganizationController] create called by userId: ${userId} with body:`,
      req.body,
    );
  ...
  getAll = asyncHandler(async (req: Request, res: Response) => {
    ...
    console.log(
      `[OrganizationController] getAll success, found ${orgs.length} orgs`,
    );
    res
      .status(200)
  ...
  update = asyncHandler(async (req: Request, res: Response) => {
    const orgId = (req.params.organizationId as string) || req.organizationId!;
    console.log(
      `[OrganizationController] update called for orgId: ${orgId} with body:`,
      req.body,
    );
  ...
  delete = asyncHandler(async (req: Request, res: Response) => {
    ...
    const userId = req.user!.id as string;
    console.log(
      `[OrganizationController] delete called for orgId: ${orgId} by userId: ${userId}`,
    );
