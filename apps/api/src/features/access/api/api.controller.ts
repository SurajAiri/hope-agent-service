...
export class ApiController {
  createApiKey = asyncHandler(async (req: Request, res: Response) => {
    ...
    console.log(
      `[ApiController] createApiKey called for orgId: ${orgId} by creatorId: ${creatorId} name: ${name}`,
    );
    ...
    res
      ...
      .json(
        new ApiResponse(
          201,
          apiKey,
          "API Key created successfully. Store it safely, it won't be shown again.",
        ),
      );
  });
  ...
  listApiKeys = asyncHandler(async (req: Request, res: Response) => {
    ...
    console.log(
      `[ApiController] listApiKeys success, found ${keys.length} keys`,
    );
    res
      .status(200)
  ...
  revokeApiKey = asyncHandler(async (req: Request, res: Response) => {
    ...
    console.log(
      `[ApiController] revokeApiKey called for orgId: ${orgId}, keyId: ${keyId} by revokerId: ${revokerId}`,
    );
